...
Page({
  ...
  onLoad: function (option) {
    ...
    this.setData({
      ...
      productGoodId: option.productGoodId,
      fromType:option.fromType,//0:付完款就返回页面   //1：付完款就重定向到该页面
      sellerId: option.sellerId,
      actId: option.actId,
      number: option.number,
      actGroupId: option.actGroupId
    })

  },
  reqIndentDetail: function () {
    var that = this;
    var url = host.host + '/order/pintuan-' + this.data.productId + '-' + this.data.productGoodId + '-' + this.data.sellerId + '-' + this.data.actId + '-' + this.data.number + '-' + this.data.actGroupId+'.html'
   
    if (address != -1) {
      url = host.host + '/order/pintuan-' + this.data.productId + '-' + this.data.productGoodId + '-' + this.data.sellerId + '-' + this.data.actId + '-' + this.data.number + '-' + this.data.actGroupId +'.html?addressId=' + address;
    }
    ...
    util.httpsGetWithId(url, function (data) {
      ...
      var good = {
        ...
        img: config + data.actPintuan.image,
        goodsId: data.actPintuan.productId,
        goodsName: data.actPintuan.name,
        price: common.toDecimal(data.actPintuan.marketPrice),
      ...
      that.setData({
        address: address,
        good: good,
        originalPrice: common.toDecimal(common.accMul(data.actPintuan.marketPrice, data.number)),
        actGroupPrice: common.toDecimal(common.accMul(data.actPintuan.price, data.number)),
        total: common.toDecimal(common.accAdd(common.accMul(data.actPintuan.price, data.number), data.transFee)),
        // 运费
  ...
  pay: function (e) {
    ...
    util.showModal('提示', '请确认订单无误', '取消', '确定', function () {
      var url = host.host + '/order/ordercommitforpintuan.html';
        //发票
        var invoiceStatus = '';
        if (that.data.ticket != '') {
          invoiceStatus = 1;
        ...
        var data = {
          ...
          actPintuanActivityId: that.data.actGroupId,
          actPintuanId: that.data.actId,
        ...
        util.httpsPostWithId(url, data, function (data) {
          if (data.data != null) {
            if (data.success) {
              ...
              var orderSn = data.data.ordersList[0].orderSn;
              ...
              util.reqPay(paySn, money, describe, function () {
                    ...
                    util.showMsg('支付成功!',function(){
                        ...
                        var url = host.host + '/order/getPintuan.html'
                        var jsData = {
                          orderSn: orderSn
                        }
                        util.httpsPostWithId(url, jsData, function (data) {
                      
                          var tuanIds = data.data.orders.pintuanId.split('-');
                        
                        
                            var pages = getCurrentPages();
                            for (var x = 0; x < pages.length;x++){
                              if (pages[x].route == dict.pages.sharePintuan){
                                console.log('已经有了这个页面了，需要回退' + (pages.length - x - 1))
                                
                                onfire.fire('refreshPage', tuanIds[1]);
                                onfire.fire('rfMyPintuan', -2);
                                if (that.data.actGroupId != 0 && that.data.actGroupId != tuanIds[1]){
                                  util.showMsg('来迟一步,该团已满,已经为您开启了新的一轮拼团,赶紧呼朋唤友来加入拼团吧!~', function () {
                                    wx.navigateBack({
                            ...
                            wx.redirectTo({
                              url: dict.pages.sharePintuan+'?actId=' + tuanIds[1],
                            })
                      
                        }.bind(this), function () { }, "获取订单活动")
                ...
                util.showModal('提示', '已取消支付,当前已生成订单，是否前往订单列表', '返回', '订单列表',function(){
                  wx.redirectTo({
