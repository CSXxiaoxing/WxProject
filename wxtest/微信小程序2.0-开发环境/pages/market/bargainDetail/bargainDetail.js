...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      productId: productId,
      actId: actId,
      currentPageUrl: '/' + pages[pages.length - 1].route + '?productId='+productId+'&actId=' + options.actId + (fromType ? '&fromType=' + fromType : ''),
    })
  ...
  //发起砍价
  startBargin: function (e) {
    wx.showLoading({
      title: '加载中..',
      mask: true,
    })
    let url = host.host + '/bargain/toBargain.html';
      ...
      let data = {
        ...
        productGoodsId: this.data.good_data.goodId //货品id
      }
      util.httpsPostWithId(url,data,function(data){
        ...
        if(data.success){
          ...
          util.navigateTo('/pages/shareBargain/shareBargain?registrationId=' + data.data.registrationId + '&sellerId=' + this.data.good_data.shopId+'&actId='+this.data.actId)
  ...
  judgeAction:function(e){
    if (this.data.good_data.isJoin == 0){
      this.setData({
      ...
      util.showModal('提示', '你已经对此商品发起过砍价,是否进入该砍价分享页面?', '返回', '砍价详情', function () {
        util.navigateTo('/pages/shareBargain/shareBargain?registrationId=' + this.data.good_data.registrationId + '&sellerId=' + this.data.good_data.shopId+'&actId='+this.data.actId)
      }.bind(this), function () {
  ...
  jumpToGoodsDetail: function (e) {
    ...
    var url = dict.pages.goods_detail + productId
    wx.redirectTo({
      url: url,
    })
  },
 ...
 */
  ...
  sel_Style: function (e) {
    ...
    var url = host.host + '/getActGoodsInfo.html';
    var data = {
    ...
    util.httpsPost(url, data, function (data) {
      ...
      if (data != null) {
        ...
        that.data.good_data.price = common.toDecimal(data.mallMobilePrice);
  ...
  Cgood: function () {
    if (this.data.good_data != undefined) {
      ...
      }, function () { }, "请求收藏商品", this.data.currentPageUrl)
    }
  },
  ...
  Cshop: function () {
    if (this.data.good_data != undefined) {
      ...
      }, function () { }, "收藏店铺", this.data.currentPageUrl)
    }
  },
  // 商品数量增加
  add: function () {
  ...
  jumpToGoodDetail: function () {
    wx.redirectTo({
      url: dict.pages.goods_detail + this.data.productId,
    })
  },
  reqService: function () { 
    var that = this;
    var url = host.host + '/bargain/' + this.data.actId + '.html'
    util.showLoading('加载中..');
    util.httpsGetWithId(url, function (data) {
      wx.hideLoading();
      if(!data.success){
      ...
      var good_data = {
        ...
        name: data.actBargain.name,//商品名字
        price: common.toDecimal(data.goods.mallMobilePrice),//商品价格
        marketPrice: common.toDecimal(data.product.marketPrice),//市场价格
      ...
      var GoodDetailImg = [];
      //详情图
      if (data.actBargain.descinfo != null && data.actBargain.descinfo != '') {
        GoodDetailImg = data.actBargain.descinfo.split(',');
      }
      //设置页面数据
      that.setData({
        good_data: good_data,
