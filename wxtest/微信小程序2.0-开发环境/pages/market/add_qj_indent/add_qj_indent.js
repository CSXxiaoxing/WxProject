...
Page({
  ...
  reqIndentDetail: function () {
    var that = this;
    var url = url = host.host + '/order/flashsale-' + this.data.productId + '-' + this.data.goodId + '-' + this.data.sellerId + '.html';
    if (address!=-1){
      var url = url = host.host + '/order/flashsale-' + this.data.productId + '-' + this.data.goodId + '-' + this.data.sellerId + '.html?addressId=' + address;
    }
    ...
    util.httpsGetWithId(url, function (data) {
      ...
      var good = {
        ...
        img: config + data.product.masterImg,
        goodsName: data.product.name1,
        price: data.actFlashSaleProduct.price,
        style: (data.productGoods != null ? data.productGoods.normName : null),
        count_num: data.number,
        total: common.accAdd(common.accMul(data.actFlashSaleProduct.price, data.number), data.transFee),
        // 运费
  ...
  pay:function(e){
    ...
    util.showModal('提示', '请确认订单无误', '取消', '确定', function () {
        var url = host.host + '/order/ordercommitforflash.html';
        //发票
        var invoiceStatus = '';
        if (that.data.ticket != '') {
          invoiceStatus = 1;
          ...
          if (data.data != null) {
            if (data.success) {
              ...
              util.reqPay(orderId, money, describe, function () {
                    ...
                    util.showModal('提示', '支付成功!已生成抢购订单，是否前往订单列表？', '前往首页', '前往订单', function () {
                      wx.redirectTo({
                ...
                util.showModal('提示', '已生成抢购订单,是否前往订单列表', '返回', '订单列表', function () {
                  wx.redirectTo({
                    url: '/pages/my_indent/my_indent',
                  })
                },function(){
