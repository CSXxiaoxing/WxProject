...
Page({
  ...
  reqIndentDetail: function () {
    var that = this;
    var url = host.host + '/cart/buynow.html'
    if (address != -1) {
      url = host.host + '/cart/buynow.html?addressId=' + address;
    }
    var data = {
      productId: this.data.productId,
      productGoodId: this.data.productGoodId,
    ...
    util.httpsPostWithId(url, data, function (data) {
      ...
      // 商品信息
      for (var i = 0; i < data.cartInfoVO.cartListVOs.length; i++) {
        var shop = {
          id: data.cartInfoVO.cartListVOs[i].seller.id,//商店id
          name: data.cartInfoVO.cartListVOs[i].seller.sellerName,//商店名称
          img: config + data.cartInfoVO.cartListVOs[i].seller.sellerLogo,//商店logo
          goods: [], //订单中的商品
          hideOrShowCoupon: true,
          coupons: [],
          couponContentSel: 0,//0是选择优惠券 1是输入序列号
          sellerAmount: data.cartInfoVO.cartListVOs[i].sellerCheckedDiscountedAmount,
        ...
        for (var x = 0; x < data.cartInfoVO.cartListVOs[i].cartList.length; x++) {
          var good = {
            id: data.cartInfoVO.cartListVOs[i].cartList[x].productId,//商品id
            goodId: data.cartInfoVO.cartListVOs[i].cartList[x].productGoods.id,//货品id
            name: data.cartInfoVO.cartListVOs[i].cartList[x].product.name1,//商品名称
            img: config + data.cartInfoVO.cartListVOs[i].cartList[x].product.masterImg,//商品图片
            count: data.cartInfoVO.cartListVOs[i].cartList[x].count,//购买数量
            price: data.cartInfoVO.cartListVOs[i].cartList[x].productGoods.mallMobilePrice,//购买单价
            style: data.cartInfoVO.cartListVOs[i].cartList[x].specInfo,//商品描述
          }
