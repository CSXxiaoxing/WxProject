...
Page({
    data: {
        key: host.agentId + dict.IDXKEY[0],
        getData: '', // 拖拽的文件
        container: [],  // 外盒style
        itSupport:'', // 技术支持
        tab: 0,
    ...
    onReady: function(){    // 首页-刷新缓存
        var that = this;
        var tab = 0;
        cl.Listener( ['Authorize'], ( obj )=>{
            !obj['key'] &&
            that.setData({
                tab: ++tab,
            })
    ...
    getUserAllData: function() {
        var that = this;
        var url = host.getHost + getCurrentPages()[0].route;    // pages/index0/index0
    ...
    setList(list){
        ...
        console.info('index0 - 数据：',list.pages1)
